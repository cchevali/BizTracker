...
## 2026-04-17
- Added `scripts/researched-listings-2026-04-17-requested.data.ts` and `scripts/researched-listings-2026-04-17-requested.lib.ts` for the next requested public-listing batch, now containing the Wayne, Clifton, and Tampa outdoor-services additions under their live canonical source pages.
- Added first-pass underwriting for `Established Landscaping, Snow Plowing, Hardscape & Concrete Company`, using the current live BizBuySell relist URL with the exact same title and location after the user-supplied `2491024` URL returned inaccessible from this environment.
- Explicitly skipped the requested Wayne County, MI and Tampa Bay, FL adds rather than approximating from similar listings because their exact requested BizBuySell URLs were not accessible.
- Updated `scripts/backfill-acquisition-thesis.lib.ts`, `scripts/verify-biztracker-reconciliation.ts`, and the researched-batch tests to include the new 2026-04-17 requested batch.
- Reconciled and re-verified production, bringing the live tracker to `77 total / 70 active / 7 passed`, with the new Clifton listing present exactly once in the production database.
- Added a `PipelineBucket` enum plus `pipelineBucket` and `publicSourceVerified` fields on `Business`, then updated the repository, filter parsing, export metadata, and dashboard/detail UI to support a cleaner pipeline model.
- Added `src/features/businesses/domain/business-source.ts` plus `scripts/thesis-realignment-2026-04-17.data.ts` and `scripts/thesis-realignment-2026-04-17.lib.ts` for the conservative brother-local / buyer-remote realignment pass.
- Reweighted automatic thesis scoring around brother-operator fit, schedule control, transferability, financeability, recurring revenue, and scenario cash, while explicitly penalizing platform dependence, weak post-brother cash, subscale economics, and unverified public sources.
- Fixed the AI-resistance rubric so pool, landscaping, janitorial, and similar physical service businesses do not get misclassified by incidental words inside note text.
- Reconciled production again after the pipeline realignment, normalizing category casing, moving weak/off-thesis rows into `WATCHLIST`, `COMP_ONLY`, or `UNVERIFIED`, and bringing live production to `77 total / 32 active / 14 watchlist / 25 comp-only / 6 unverified / 29 passed`.
- Expanded the 2026-04-17 requested managed batch to add `Established Landscaping & Snow Removal Company | $500K SDE | 30+ Years` under the live direct BizQuest individual page `BW2480416` and `Scalable Landscaping Platform | 50% Recurring Revenue | Tampa` under the live BizBuySell page `2479308`, while keeping Clifton normalized to the live `2445240` BizBuySell relist.
- Documented that the user-provided Wayne BizQuest URL `BW2487125` is stale or incorrect in the live marketplace and should not be treated as the canonical Wayne source.
- Reconciled and re-verified production after the Wayne/Tampa patch, bringing the live tracker to `79 total / 34 active / 14 watchlist / 25 comp-only / 6 unverified / 29 passed`, with Wayne scored `89`, Clifton `88`, and Tampa `79` in the active bucket.
...
- Updated the 2026-04-23 landscaping overrides again after follow-up direction so those five verified rows now live in `ACTIVE` instead of `COMP_ONLY`, while still preserving the Texas listing's `LETTER_OF_INTENT` pending status.
- Reconciled production again, bringing the live tracker to `84 total / 38 active / 14 watchlist / 26 comp-only / 6 unverified / 29 passed`, and confirmed the default active view now surfaces the new landscaping rows.
