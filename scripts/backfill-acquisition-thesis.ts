...
async function archiveLowFitBusinesses() {
...
async function createMissingPublicListings() {
  const businesses = await loadBusinesses();
  ...
  for (const seed of newListingSeeds) {
    ...
    if (
      knownSourceUrls.has(seed.sourceUrl) ||
      knownTitles.has(normalizedSeedTitle)
    ) {
      continue;
    }
...
async function backfillExistingBusinesses() {
  const businesses = await loadBusinesses();
  let updatedCount = 0;
  ...
  for (const [businessName, spec] of Object.entries(existingBackfills)) {
    ...
    if (await applyBackfill(business, spec)) {
      updatedCount += 1;
    }
  }
...
async function backfillNewListings() {
  const businesses = await loadBusinesses();
  let updatedCount = 0;
  ...
  for (const seed of newListingSeeds) {
    ...
    if (await applyBackfill(business, seed.spec, seed.sourceUrl)) {
      updatedCount += 1;
    }
  }
...
async function main() {
  const archivedNames = await archiveLowFitBusinesses();
  const createdNames = await createMissingPublicListings();
  const existingUpdatedCount = await backfillExistingBusinesses();
  const newListingUpdatedCount = await backfillNewListings();

  console.log(`Archived or confirmed passed: ${archivedNames.length}`);
  console.log(`Created new listings: ${createdNames.length}`);
  console.log(`Updated existing active businesses: ${existingUpdatedCount}`);
  console.log(`Updated or confirmed new listings: ${newListingUpdatedCount}`);

  if (archivedNames.length > 0) {
    console.log("Archived businesses:");
    for (const businessName of archivedNames) {
  ...
  if (createdNames.length > 0) {
    ...
    for (const businessName of createdNames) {
      console.log(`- ${businessName}`);
    }
  }
}
